...
@frontend.route('/')
async def home():
    vn = await glob.db.fetchrow('SELECT pp, users.name, time FROM scores LEFT JOIN users ON scores.uid = users.id LEFT JOIN maps ON scores.md5 = maps.md5 WHERE users.priv & 1 > 0 AND maps.status = 2 AND scores.status = 2 AND scores.mode = 0 ORDER BY pp DESC LIMIT 1')
    rx = await glob.db.fetchrow('SELECT pp, users.name, time FROM scores_rx LEFT JOIN users ON scores_rx.uid = users.id LEFT JOIN maps ON scores_rx.md5 = maps.md5 WHERE users.priv & 1 > 0 AND maps.status = 2 AND scores_rx.status = 2 AND scores_rx.mode = 0 ORDER BY pp DESC LIMIT 1')
    ap = await glob.db.fetchrow('SELECT pp, users.name, time FROM scores_ap LEFT JOIN users ON scores_ap.uid = users.id LEFT JOIN maps ON scores_ap.md5 = maps.md5 WHERE users.priv & 1 > 0 AND maps.status = 2 AND scores_ap.status = 2 AND scores_ap.mode = 0 ORDER BY pp DESC LIMIT 1')
    # vnc = await glob.db.fetchrow('SELECT pp, users.name, play_time FROM scores_vn_cheat LEFT JOIN users ON scores_vn_cheat.userid = users.id LEFT JOIN maps ON scores_vn_cheat.map_md5 = maps.md5 WHERE users.priv & 1 AND users.frozen = 0 AND scores_vn_cheat.status = 2 AND scores_vn_cheat.mode = 0 AND maps.status IN (2,3,4,5) ORDER BY pp DESC LIMIT 1')
    # rxc = await glob.db.fetchrow('SELECT pp, users.name, play_time FROM scores_rx_cheat LEFT JOIN users ON scores_rx_cheat.userid = users.id LEFT JOIN maps ON scores_rx_cheat.map_md5 = maps.md5 WHERE users.priv & 1 AND users.frozen = 0 AND scores_rx_cheat.status = 2 AND scores_rx_cheat.mode = 0 AND maps.status IN (2,3,4,5) ORDER BY pp DESC LIMIT 1')
    # apc = await glob.db.fetchrow('SELECT pp, users.name, play_time FROM scores_ap_cheat LEFT JOIN users ON scores_ap_cheat.userid = users.id LEFT JOIN maps ON scores_ap_cheat.map_md5 = maps.md5 WHERE users.priv & 1 AND users.frozen = 0 AND scores_ap_cheat.status = 2 AND scores_ap_cheat.mode = 0 AND maps.status IN (2,3,4,5) ORDER BY pp DESC LIMIT 1')
    try:
